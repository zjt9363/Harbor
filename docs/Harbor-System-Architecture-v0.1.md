...
API Server 是 Harbor 的平台业务后端，负责：
...
OpenClaw 所在层是 Harbor 的 Agent 运行层，负责：
...
这一层处于平台业务层和模型推理层之间，是 Harbor 的智能能力承载层。
...
Harbor 第一阶段应设计为一个“桌面客户端 + 中心化 Agent 服务”的混合架构：客户端负责用户交互和本地工作目录接入，服务端负责会话、数据、Agent 运行和模型推理。
